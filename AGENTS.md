...
- Check the declared type before adding presence checks. Do not assert fields that are already non-nullable (for example, `Company.id`, `Company.name`, `Company.__typename`, or `Account.fullName`). For values that may be `null` or `undefined`, prefer safe handling when the feature can continue correctly: `company.admins ?? []` for optional lists, `permissions?.canCreateSpace ?? false` for permissions, or `if (!generalSpace) return null` only when hiding that component is valid. Use `assertPresent(value, message)` only when the value may be absent, is essential to the operation, and has no safe fallback; prefer it over an inline null check that throws. Do not use the non-null assertion operator (`!`) to bypass nullable types.
