...
describe("truncateCompanyName", () => {
  ...
  it("leaves the name unchanged when Intl.Segmenter is unavailable", () => {
    ...
    try {
      const longName = "Nexus Global Manufacturing Group";
      const nameWithFlag = `${"A".repeat(22)}🇺🇸XX`;

      expect(truncateCompanyName(longName)).toBe(longName);
      expect(truncateCompanyName(nameWithFlag)).toBe(nameWithFlag);
    } finally {
