...
export function useTaskKeyboardNavigation<TElement extends HTMLElement>() {
  ...
  const clearSelection = React.useCallback(() => {
    ...
    if (document.activeElement instanceof HTMLElement && document.activeElement.matches(TASK_ROW_SELECTOR)) {
  ...
  React.useEffect(() => {
    const handleKey = (event: KeyboardEvent, direction: Direction) => {
      ...
      if (!isInNavigationScope(event, containerRef.current)) return;
  ...
  }, [clearSelection, selectTask]);

  return { containerRef, selectedTaskId, clearSelection } as const;
}
...
function isInNavigationScope(event: KeyboardEvent, container: HTMLElement | null): boolean {
  ...
  return isNeutralDocumentTarget(target) && isNeutralActiveElement(activeElement);
}
