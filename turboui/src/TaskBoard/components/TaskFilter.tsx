...
const filterOptions: FilterOption[] = [
  ...
  { type: "milestone", label: "Milestone", icon: <IconTarget size={14} />, operators: ["is", "is_not"], hasSubmenu: true },
  ...
  { type: "created_date", label: "Created date", icon: <IconCalendar size={14} />, operators: ["before", "after", "between"] },
  { type: "updated_date", label: "Updated date", icon: <IconCalendar size={14} />, operators: ["before", "after", "between"] },
  { type: "started_date", label: "Started date", icon: <IconCalendar size={14} />, operators: ["before", "after", "between"] },
  { type: "completed_date", label: "Completed date", icon: <IconCalendar size={14} />, operators: ["before", "after", "between"] },
];
            ...
            {!selectedOption ? (
              ...
              <div className="p-3">
                ...
                <div className="relative mb-3">
                  <IconSearch size={14} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-content-subtle" />
                  <input
                    ref={searchInputRef}
                    type="text"
...
export function FilterBadges({ filters, onFiltersChange }: { filters: Types.FilterCondition[]; onFiltersChange: (filters: Types.FilterCondition[]) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      {filters.map((filter) => (
        <div
          key={filter.id}
          className="flex items-center gap-1 px-2 py-1 bg-surface-accent rounded text-sm text-content-base"
        >
          <span>{filter.label}</span>
          <button
            onClick={() => removeFilter(filter.id)}
            className="hover:text-content-error"
          >
            <IconX size={12} />
          </button>
        </div>
