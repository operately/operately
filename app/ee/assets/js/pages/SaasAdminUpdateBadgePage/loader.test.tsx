...
it("prefetches query inputs and reuses cached data on mount and reentry", async () => {
  jest.mocked(axios.get).mockResolvedValue({ data: { enabled: false } });
...
it("observes refreshed query data", async () => {
  ...
  await act(() => queryClient.invalidateQueries({ queryKey: AdminApi.getUpdateBadgeSettingsQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.enabled).toEqual(updated.enabled));
});
