...
it("prefetches query inputs and reuses cached data on mount and reentry", async () => {
  jest.mocked(axios.get).mockResolvedValue({ data: { sources: [] } });
  ...
  expect(result.current).toEqual({ sources: [] });
  await loader();
...
it("observes refreshed query data", async () => {
  ...
  const { result } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  const updated = { sources: [{ id: "updated" }] };
  jest.mocked(axios.get).mockResolvedValue({ data: updated });
  await act(() => queryClient.invalidateQueries({ queryKey: AdminApi.getSearchIndexStatusQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.sources).toEqual(updated.sources));
});
