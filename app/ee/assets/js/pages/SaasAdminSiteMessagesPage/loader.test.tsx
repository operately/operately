...
it("prefetches query inputs and reuses cached data on mount and reentry", async () => {
  jest.mocked(axios.get).mockResolvedValue({ data: { messages: [] } });
...
it("observes refreshed query data", async () => {
  ...
  const { result } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  const updated = { messages: [{ id: "updated" }] };
  jest.mocked(axios.get).mockResolvedValue({ data: updated });
  await act(() => queryClient.invalidateQueries({ queryKey: AdminApi.listSiteMessagesQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.messages).toEqual(updated.messages));
});
