...
it("prefetches query inputs and reuses cached data on mount and reentry", async () => {
  jest.mocked(axios.get).mockResolvedValue({ data: { emailSettings: null } });
  ...
  expect(result.current).toEqual({ emailSettings: null });
  await loader();
...
it("observes refreshed query data", async () => {
  ...
  const { result } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  const updated = { emailSettings: { provider: "smtp" } };
  jest.mocked(axios.get).mockResolvedValue({ data: updated });
  await act(() => queryClient.invalidateQueries({ queryKey: AdminApi.getEmailSettingsQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.emailSettings).toEqual(updated.emailSettings));
});
