defmodule OperatelyEE.AdminApi.Mutations.UnarchiveBillingPlanDefinition do
  ...
  def call(_conn, inputs) do
    if not Billing.billing_enabled?() do
      ...
      with {:ok, id} <- decode_id(inputs.id),
           ...
           {:ok, unarchived} <- Billing.unarchive_plan_definition(plan_definition) do
        {:ok, %{plan_definition: OperatelyWeb.Api.Serializer.serialize(unarchived, level: :full)}}
      else
        {:error, error, message} ->
          {:error, error, message}
