...
it("fetches fresh data after invalidation and re-entry", async () => {
  ...
  jest.mocked(axios.get).mockResolvedValue({
    data: {
      goal: { id: "goal-1", name: "After", access_levels: { public: 0, company: 10, space: 40 } },
    },
  });
...
it("rejects missing access levels", async () => {
  jest.mocked(axios.get).mockResolvedValue({ data: { goal: { id: "goal1" } } });
  const error = jest.spyOn(console, "error").mockImplementation(() => {});
  try {
    await expect(readLoadedData(await visit())).rejects.toThrow(/access levels/i);
  } finally {
    error.mockRestore();
  }
});
