/** @jest-environment <rootDir>/../turboui/node_modules/jest-environment-jsdom */
import React from "react";
import axios from "axios";
import Api from "@/api";
import { queryClient } from "@/api/queryClient";
import { QueryClientProvider } from "@tanstack/react-query";
import * as Pages from "@/components/Pages";
import { act, renderHook, waitFor } from "@/__tests__/renderHook";
import { loader, useLoadedData } from "./loader";

jest.mock("axios");
jest.mock("turboui", () => ({}));
jest.mock("@/components/Pages", () => ({ useLoadedData: jest.fn() }));
jest.mock("react-router", () => ({}));

const template = {
  id: "template1",
  name: "Original",
  space: { id: "space1" },
  resourceNodes: [
    { id: "node1", type: "document", document: { id: "child1", name: "Resource", blob: { url: "/file" } } },
  ],
};
const response = { template, discussion: { id: "node1", title: "Notes", body: "" }, comments: [] };
const wrapper = ({ children }: React.PropsWithChildren) => (
  <QueryClientProvider client={queryClient}>{children}</QueryClientProvider>
);

beforeEach(() => {
  queryClient.clear();
  jest.clearAllMocks();
  Api.default.setBasePath("/api/v2");
  Api.default.setHeaders({});
  jest.mocked(axios.get).mockResolvedValue({ data: response });
});

afterEach(() => queryClient.clear());

async function visit() {
  const args = {
    params: { templateId: "template1", id: "node1" },
    request: { url: "http://localhost/new?folderId=folder1&type=figma" },
  };
  const inputs = await loader(args);
  jest.mocked(Pages.useLoadedData).mockReturnValue(inputs);

  return inputs;
}

it("reuses prefetched data on mount and cached route re-entry", async () => {
  const inputs = await visit();
  expect(inputs).not.toHaveProperty("template");

  const { result, unmount } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  expect(result.current.template.name).toBe("Original");
  unmount();

  await visit();
  renderHook(useLoadedData, { initialProps: undefined, wrapper });
  expect(axios.get).toHaveBeenCalledTimes(3);
});

it("subscribes to invalidation and retains cached data after a background failure", async () => {
  await visit();
  const { result } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  jest.mocked(axios.get).mockResolvedValue({ data: { ...response, template: { ...template, name: "Updated" } } });

  await act(() => queryClient.invalidateQueries({ queryKey: Api.project_templates.getQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.template.name).toBe("Updated"));

  jest.mocked(axios.get).mockRejectedValue(new Error("Offline"));
  await act(() => queryClient.invalidateQueries());
  expect(result.current.template.name).toBe("Updated");
});

it.each([null, { ...template, space: null }])("guards missing template data or its space", async (value) => {
  await visit();
  queryClient.setQueryData(Api.project_templates.getQueryKey({ id: "template1" }), { template: value });
  const log = jest.spyOn(console, "error").mockImplementation(() => {});

  try {
    expect(() => renderHook(useLoadedData, { initialProps: undefined, wrapper })).toThrow(/Template/);
  } finally {
    log.mockRestore();
  }
});

it("subscribes to comments independently", async () => {
  await visit();
  const { result } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  jest.mocked(axios.get).mockResolvedValue({ data: { comments: [{ id: "comment1" }] } });

  await act(() => queryClient.invalidateQueries({ queryKey: Api.project_templates.listCommentsQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.comments).toEqual([{ id: "comment1" }]));
});

it("prefetches independent requests in parallel and subscribes to the discussion", async () => {
  const finish: ((value: unknown) => void)[] = [];
  jest.mocked(axios.get).mockImplementation(() => new Promise((resolve) => finish.push(resolve)));
  const visiting = visit();

  await waitFor(() => expect(finish).toHaveLength(3));
  finish.forEach((resolve) => resolve({ data: response }));
  await visiting;

  const { result } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  expect(result.current.discussion.body).toBe("");
  jest.mocked(axios.get).mockResolvedValue({ data: { discussion: { id: "node1", title: "Updated" } } });

  await act(() => queryClient.invalidateQueries({ queryKey: Api.project_templates.getDiscussionQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.discussion.title).toBe("Updated"));
});

it("guards missing comment data", async () => {
  const { commentsInput } = await visit();
  queryClient.setQueryData(Api.project_templates.listCommentsQueryKey(commentsInput), { comments: null });
  const log = jest.spyOn(console, "error").mockImplementation(() => {});

  try {
    expect(() => renderHook(useLoadedData, { initialProps: undefined, wrapper })).toThrow(/comments/);
  } finally {
    log.mockRestore();
  }
});

it("guards missing discussion data", async () => {
  const { discussionInput } = await visit();
  queryClient.setQueryData(Api.project_templates.getDiscussionQueryKey(discussionInput), { discussion: null });
  const log = jest.spyOn(console, "error").mockImplementation(() => {});

  try {
    expect(() => renderHook(useLoadedData, { initialProps: undefined, wrapper })).toThrow(/discussion/);
  } finally {
    log.mockRestore();
  }
});
