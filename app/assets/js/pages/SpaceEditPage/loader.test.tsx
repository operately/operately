...
it("fetches fresh data after invalidation and re-entry", async () => {
  jest.mocked(axios.get).mockResolvedValue({
    data: { space: { id: "space-1", name: "Before" } },
  });
  await visit();
  await queryClient.invalidateQueries({ queryKey: Api.spaces.getQueryKeyPrefix() });
  await visit();
