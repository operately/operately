...
it("reuses prefetched data on mount and route re-entry", async () => {
  ...
  expect(inputs).not.toHaveProperty("spaces");
  ...
  expect(result.current.spaces).toEqual(response.spaces);
  unmount();
  await loader();
...
it("refreshes mounted data after invalidation", async () => {
  ...
  const updated = [{ id: "space1", name: "Space" }];
  jest.mocked(axios.get).mockResolvedValue({ data: { ...response, spaces: updated } });
  await act(() => queryClient.invalidateQueries({ queryKey: Api.spaces.listQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.spaces).toEqual(updated));
});
