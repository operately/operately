...
it("subscribes to refreshed cache data and retains it after a failed background request", async () => {
  ...
  jest.mocked(Pages.useLoadedData).mockReturnValue(inputs);
  const { result } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  const queryKey = Api.spaces.getQueryKey(inputs.spaceInput);
  jest.mocked(axios.get).mockResolvedValue({
    data: {
      space: { id: "resource1", name: "Changed" },
      discussions: [{ id: "discussion1" }],
      my_drafts: [{ id: "draft1" }],
    },
  });
  ...
  await act(() =>
    queryClient.invalidateQueries({ queryKey: Api.spaces.listDiscussionsQueryKey(inputs.discussionsInput) }),
  );
  await waitFor(() => expect(result.current.myDrafts).toEqual([{ id: "draft1" }]));
  expect(result.current.discussions).toEqual([{ id: "discussion1" }]);
...
it("rejects an unavailable discussion list even when the space is cached", async () => {
  jest.mocked(axios.get).mockResolvedValue({ data: { space: { id: "resource1" } } });
  const inputs = await loader({ params: { id: "resource1" } });
  jest.mocked(Pages.useLoadedData).mockReturnValue(inputs);
  const error = jest.spyOn(console, "error").mockImplementation(() => {});
  try {
    expect(() => renderHook(useLoadedData, { initialProps: undefined, wrapper })).toThrow(
