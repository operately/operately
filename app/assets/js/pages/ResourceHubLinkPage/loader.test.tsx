...
const response = { link: resource, subscribed: false };
...
it("subscribes to prefetched data without duplicate mount or route re-entry requests", async () => {
  ...
  expect(inputs).not.toHaveProperty("link");

  const { result, unmount } = renderHook(useLoadedData, { initialProps: undefined, wrapper });
  expect(result.current.link.name).toBe("Original");
...
it("updates on invalidation and retains data after a failed background refresh", async () => {
  ...
  jest.mocked(axios.get).mockResolvedValue({ data: { ...response, link: { ...resource, name: "Updated" } } });
  await act(() => queryClient.invalidateQueries({ queryKey: Api.links.getQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.link.name).toBe("Updated"));

  jest.mocked(axios.get).mockRejectedValue(new Error("Offline"));
  await act(() => queryClient.invalidateQueries({ queryKey: Api.links.getQueryKeyPrefix() }));

  expect(result.current.link.name).toBe("Updated");
});

it("guards missing resource data", async () => {
  const inputs = await visit();
  queryClient.setQueryData(Api.links.getQueryKey(inputs.linkInput), { link: null });
...
it("prefetches subscription status alongside the resource", async () => {
  ...
  jest.mocked(axios.get).mockResolvedValue({ data: { subscribed: true } });
  await act(() => queryClient.invalidateQueries({ queryKey: Api.notifications.isSubscribedQueryKeyPrefix() }));
  await waitFor(() => expect(result.current.isCurrentUserSubscribed).toBe(true));
});
