...
it.each([false, true])(
  ...
  async (canComment) => {
    ...
    const loaded = {
      checkIn: { id: "resource1", reactions, project: { id: "project1", permissions: { canComment } } },
    ...
    try {
      ...
      loaded.checkIn.reactions = [{ __typename: "reaction", id: "reaction2", emoji: "🎉", person: null }];
      render();
      expect(jest.mocked(useOptimisticReactions).mock.calls.at(-1)?.[0].initialReactions).toEqual(
        loaded.checkIn.reactions,
      );
    } finally {
      act(() => root.unmount());
      client.clear();
