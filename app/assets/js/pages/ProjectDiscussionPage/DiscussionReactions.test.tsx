...
it.each([false, true])(
  ...
  async (canComment) => {
    ...
    const loaded = {
      discussion: { id: "resource1", reactions, project: { id: "project1" }, projectPermissions: { canComment } },
    ...
    try {
      ...
      loaded.discussion.reactions = [{ __typename: "reaction", id: "reaction2", emoji: "🎉", person: null }];
      render();
      expect(jest.mocked(useOptimisticReactions).mock.calls.at(-1)?.[0].initialReactions).toEqual(
        loaded.discussion.reactions,
      );
    } finally {
      act(() => root.unmount());
      client.clear();
