...
export function ApproachingLimitBanner() {
  ...
  const banner = React.useMemo(() => {
    ...
    const dangerBanner = Billing.buildBillingDangerBanner(billingAccessState, billingLimitWarnings, canManageBilling, {
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {banner.usageRows.map((row) => (
                ...
                >
                  <span className="font-semibold text-content-accent">{row.label}:</span>{" "}
                  {row.value}
                </div>
              ))}
            </div>
