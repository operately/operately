...
export type MilestoneForOrdering = {
  id: string;
  [key: string]: any;
};

export interface UseProjectMilestoneOrderingOptions {
  projectId: string;
  cacheKey: string;
  refresh?: () => Promise<void>;
  initialMilestones: MilestoneForOrdering[];
  initialOrderingState: string[];
}

export interface UseProjectMilestoneOrderingResult {
  milestones: MilestoneForOrdering[];
  setMilestones: React.Dispatch<React.SetStateAction<MilestoneForOrdering[]>>;
...
// useProjectMilestoneOrdering caches the original milestones/order in a ref so normalization only happens once, then seeds React state for ordered milestones and the current ordering array.
// orderingRef mirrors the latest order to keep synchronous reads in callbacks aligned with async React updates, while setMilestones normalizes incoming lists and only mutates ordering state when values actually change.
// reorderMilestones performs optimistic moves: it shifts the requested id, updates UI state immediately, calls the API, reconciles with the server response, and rolls back with a toast on failure while also invalidating cache/refreshing when successful.
export function useProjectMilestoneOrdering({
  ...
  const initialDataRef = React.useRef<{ milestones: MilestoneForOrdering[]; ordering: string[] } | null>(null);
  ...
  const [milestones, setMilestonesState] = React.useState<MilestoneForOrdering[]>(initialDataRef.current.milestones);
  ...
  const setMilestones = React.useCallback<React.Dispatch<React.SetStateAction<MilestoneForOrdering[]>>>(
    (update) => {
      setMilestonesState((prev) => {
        const next =
          typeof update === "function" ? (update as (value: MilestoneForOrdering[]) => MilestoneForOrdering[])(prev) : update;
  ...
  const reorderMilestones = React.useCallback(
    async ({ sourceId, destinationIndex }: { sourceId: string; destinationIndex: number }) => {
      ...
      try {
        ...
        console.error("Failed to reorder project milestones", error);
...
// normalizeMilestoneOrdering builds a deduped sequence that preserves known ids, discards stale ones, and appends unseen milestones at the end so data is never dropped.
function normalizeMilestoneOrdering(ordering: string[] | undefined, milestones: MilestoneForOrdering[]): string[] {
...
// reorderMilestonesByIds converts the normalized ordering back into milestone objects, gracefully skipping ids that are no longer present in the map.
function reorderMilestonesByIds(
  milestones: MilestoneForOrdering[],
  ordering: string[],
): MilestoneForOrdering[] {
  if (milestones.length === 0) {
    return [];
  }
  ...
  return normalized
    .map((id) => map.get(id))
    .filter((milestone): milestone is MilestoneForOrdering => Boolean(milestone));
}
