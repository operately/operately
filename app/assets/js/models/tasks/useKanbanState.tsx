...
export function useKanbanState(options: UseKanbanStateOptions) {
  ...
  const handleTaskKanbanChange = React.useCallback(
    async (event: TaskKanbanChangeEvent) => {
      await persistTaskKanbanChange(event);
    },
...
export function buildTaskStatusChangeKanbanEvent({
  ...
  const destinationIndex = isCompletedStatus(statusOption) ? 0 : withoutTask[destinationStatus]?.length || 0;
...
function isCompletedStatus(status: StatusOption): boolean {
  return status.closed === true && status.color === "green";
}
