defmodule OperatelyWeb.Paths do
  ...
  def invite_join_full_path(token) when is_binary(token) do
    create_path(["join", token, "full"])
  end
  ...
  def space_discussions_drafts_path(company = %Company{}, space = %Group{}) do
    create_path([company_id(company), "spaces", space_id(space), "discussions", "drafts"])
  end
  ...
  def company_billing_path(company = %Company{}) do
  ...
  def project_template_id(id) when is_binary(id), do: Operately.ShortUuid.encode!(id)
  def project_template_id(%{id: id}), do: project_template_id(id)

  def project_template_milestone_id(%{id: id}), do: project_template_milestone_id(id)
  def project_template_milestone_id(id) when is_binary(id), do: Operately.ShortUuid.encode!(id)

  def project_template_task_id(%{id: id}), do: project_template_task_id(id)
  def project_template_task_id(id) when is_binary(id), do: Operately.ShortUuid.encode!(id)

  def project_template_person_id(%{id: id}), do: project_template_person_id(id)
  def project_template_person_id(id) when is_binary(id), do: Operately.ShortUuid.encode!(id)
