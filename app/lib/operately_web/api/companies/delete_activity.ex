defmodule OperatelyWeb.Api.Companies.DeleteActivity do
  ...
  import Operately.Access.Filters, only: [filter_by_admin_access: 2, filter_by_view_access: 2]
  ...
  def call(conn, inputs) do
    with {:ok, activity_id} <- decode_id(inputs.activity_id),
         {:ok, activity} <- load_activity(activity_id, me(conn)),
  ...
  defp load_activity(activity_id, person) do
    activity =
      from(a in Activity,
        where: a.id == ^activity_id,
        where: fragment("? ->> ? = ?", a.content, "company_id", ^person.company_id)
      )
