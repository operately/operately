  ...
  defp render_activity(activity) do
    ...
    case activity.action do
      "goal_check_toggled" ->
        ...
        item_name = content["name"] || "Unknown Item"
        "- **#{timestamp}** - #{author} #{status} checklist item \"#{item_name}\""
    ...
    e ->
      "- **#{format_timestamp(activity.inserted_at)}** - Error rendering activity: #{inspect(e)}"
  end
