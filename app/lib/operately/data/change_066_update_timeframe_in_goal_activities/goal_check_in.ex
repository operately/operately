defmodule Operately.Data.Change066UpdateTimeframeInGoalActivities.GoalCheckIn do
  import Ecto.Query
  alias Operately.Repo
  alias Operately.Activities.Activity

  def run do
    goal_check_in_activities = Repo.all(
      from a in Activity,
      where: a.action == "goal_check_in",
      select: a
    )

    {success_count, error_count} = Enum.reduce(goal_check_in_activities, {0, 0}, fn activity, {success, error} ->
  ...
  defp update_activity_timeframes(activity) do
    old_timeframe = Map.get(activity.content, "old_timeframe")
    new_timeframe = Map.get(activity.content, "new_timeframe")
