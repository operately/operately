defmodule Operately.Search.Query.Company do
  @moduledoc """
  ...
  alias Operately.Search.Query.Company.{ResourceHubItems, ResultBuilder}
  ...
  defp candidate_query(company_id, person_id, query) do
    full_text = FullText.build(query)
  ...
  defp matched_candidates_query(company_id, eligible_items, accessible_contexts, full_text) do
    from(entry in Entry,
      ...
      where: ^FullText.match_dynamic(full_text),
      select: %{
        entry_id: entry.id,
