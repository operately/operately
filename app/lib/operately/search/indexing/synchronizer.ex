defmodule Operately.Search.Indexing.Synchronizer do
  @moduledoc """
  ...
  defp synchronize(source_type, source_ids) do
    ...
    with {:ok, source_adapter} <- AdapterRegistry.fetch(source_type),
         ...
         {:ok, write_summary} <- Indexing.upsert_all(changes.entries),
         :ok <- validate_write_summary(write_summary),
         {:ok, deleted_count} <- Indexing.delete_many(changes.deletion_keys) do
  ...
  defp prepare_changes(source_type, source_adapter, requested_ids, source_records) do
    records_by_id = Map.new(source_records, &{Adapter.id!(&1), &1})
