defmodule Operately.Search.Maintenance.RunLifecycle do
  @moduledoc """
  ...
  These functions run inside the transaction opened by `BatchRunner`, keeping
  ...
  alias Operately.Search.Maintenance.{BatchResult, ErrorCategory, Run, Worker}
  ...
  defp lock_run!(run_id) do
    from(run in Run, where: run.id == ^run_id, lock: "FOR UPDATE")
    |> Repo.one!()
  end
  ...
  defp start_index_scan(run) do
    run
    |> Run.changeset(%{phase: :index_scan, cursor: nil})
    |> Repo.update!()
    |> enqueue_next_batch()
  end
  ...
  defp complete_run(run) do
    ...
    run
    |> Run.changeset(%{status: status, completed_at: DateTime.utc_now()})
    |> Repo.update!()
  end
