defmodule Operately.Search.Maintenance.IndexScan do
  @moduledoc """
  ...
  alias Operately.Search.Maintenance.{BatchResult, EntrySynchronizer}
  ...
  defp reconcile(source_type, source_adapter, indexed_entries, source_records) do
    ...
    source_records_by_id = Map.new(source_records, fn source_record -> {Adapter.id!(source_record), source_record} end)
    ...
    with {:ok, result} <- EntrySynchronizer.repair_locked(source_type, source_adapter, current_source_records),
         {:ok, deleted_count} <- Indexing.delete_many(entry_keys(orphaned_entries)) do
      {:ok,
       %{
         result
