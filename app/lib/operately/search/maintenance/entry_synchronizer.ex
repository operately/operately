defmodule Operately.Search.Maintenance.EntrySynchronizer do
  @moduledoc """
  ...
  alias Operately.Search.Indexing.Adapter
  alias Operately.Search.Maintenance.{BatchResult, ErrorCategory}
  ...
  def backfill(source_type, source_adapter, source_records) do
    synchronize(source_type, source_adapter, source_records, &Indexing.upsert_all_guarded/1)
  end

  def repair_locked(source_type, source_adapter, source_records) do
    synchronize(source_type, source_adapter, source_records, &Indexing.repair_all_locked/1)
  end
  ...
  defp synchronize(source_type, source_adapter, source_records, write_entries) do
    with {:ok, prepared_changes} <- prepare_index_changes(source_type, source_adapter, source_records),
         ...
         {:ok, _deleted_count} <- Indexing.delete_many(prepared_changes.deletion_keys ++ invalid_entry_keys) do
      {:ok,
       %BatchResult{
  ...
  defp prepare_index_changes(source_type, source_adapter, source_records) do
    Enum.reduce_while(source_records, {:ok, @empty_changes}, fn source_record, {:ok, changes} ->
      source_id = Adapter.id!(source_record)
