defmodule Operately.Support.Features.GoalClosingSteps do
  ...
  step :close_some_projects, ctx do
    ...
    {:ok, _} = Operately.Operations.ProjectClosed.run(ctx.creator, ctx.project_to_close, %{
  ...
  step :visit_goal_closing_page, ctx do
    path = create_goal_closing_path(ctx.company, ctx.goal)
    ctx
    |> UI.visit(path)
  ...
  step :assert_closed_projects_not_shown_in_warning, ctx do
    ctx
    |> UI.refute_text("Project To Close")  # This project was closed and shouldn't appear
  end

  step :assert_active_projects_shown_in_warning, ctx do
    ctx
    |> UI.assert_text("Active Project")  # This project is active and should appear
    |> UI.assert_text("Paused Project")  # Paused projects should also appear as active
  end
  ...
  # Helper to create the goal closing path since it's not in the Paths module
  defp create_goal_closing_path(company, goal) do
    company_id = Operately.ShortUuid.encode!(company.id)
    goal_id = Operately.ShortUuid.encode!(goal.id)
    "/#{company_id}/goals/#{goal_id}/complete"
  end
end