defmodule Operately.Support.Features.ProjectTasksSteps do
  ...
  step :open_kanban_task_slide_in, ctx, task_key do
    ...
    ctx
    |> UI.click(testid: UI.testid(["kanban-card-title", Paths.task_id(task)]))
    |> UI.wait_until_has(testid: "task-slide-in")
    |> UI.wait_until_has(testid: "task-header")
    |> UI.sleep(400)
  end

  step :close_kanban_task_slide_in, ctx do
    ctx
    |> UI.send_keys([:escape])
    |> UI.sleep(400)
  ...
  step :change_kanban_task_status, ctx, opts do
