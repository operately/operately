defmodule OperatelyWeb.Api.Folders.RenameTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:folders, :rename], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company} and space=#{@test.space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:folders, :rename], %{
  ...
  describe "functionality" do
    ...
    test "renames folder", ctx do
      ...
      assert {200, %{success: true}} = mutation(ctx.conn, [:folders, :rename], %{
