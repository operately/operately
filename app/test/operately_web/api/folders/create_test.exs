defmodule OperatelyWeb.Api.Folders.CreateTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:folders, :create], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company} and space=#{@test.space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:folders, :create], %{
  ...
  describe "functionality" do
    ...
    test "creates folder within hub", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:folders, :create], %{
