defmodule OperatelyWeb.Api.Folders.CopyTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:folders, :copy], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company}, origin_space=#{@test.origin_space} and dest_space=#{@test.dest_space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:folders, :copy], %{
  ...
  describe "functionality" do
    ...
    test "creates a copy of folder", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:folders, :copy], %{
