defmodule OperatelyWeb.Api.Folders.GetTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:folders, :get], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company} and space=#{@test.space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = query(ctx.conn, [:folders, :get], %{
  ...
  describe "get_resource_hub_test functionality" do
    ...
    test "include_nodes", ctx do
      assert {200, res} = query(ctx.conn, [:folders, :get], %{
