defmodule OperatelyWeb.Api.People.SearchTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:people, :search], query: "John", ignored_ids: [], search_scope_type: "company")
    end
    ...
    test "suspended people don't have access", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :search], query: "Doe", ignored_ids: [], search_scope_type: "company")
  ...
  describe "search_people functionality" do
    ...
    test "searches people by title", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :search], query: "Developer", ignored_ids: [], search_scope_type: "company")
    ...
    test "orders by the best prefix match on full name", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :search], query: "Smith", ignored_ids: [], search_scope_type: "company")
    ...
    test "ignoring people by id", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :search], query: "John", ignored_ids: [Paths.person_id(person1)], search_scope_type: "company")
