defmodule OperatelyWeb.Api.People.ListTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:people, :list], %{})
    end
    ...
    test "it doesn't return people from other companies", ctx do
      ...
      assert {200, %{people: people}} = query(ctx.conn, [:people, :list], %{})
  ...
  describe "permissions" do
    ...
    test "company member can query people from the same company", ctx do
      assert {200, res} = query(ctx.conn, [:people, :list], %{})
    ...
    test "company member can't query people from another company", ctx do
      ...
      assert {200, res} = query(conn2, [:people, :list], %{})
  ...
  describe "get_people functionality" do
    ...
    test "include_suspeded", ctx do
      ...
      assert {200, %{people: people}} = query(ctx.conn, [:people, :list], %{include_suspended: true})
    ...
    test "only_suspended", ctx do
      ...
      assert {200, %{people: people}} = query(ctx.conn, [:people, :list], %{only_suspended: true})
      assert find_person_in_response(people, suspended_person)
    ...
    test "include_manager", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :list], %{include_manager: true})
    ...
    test "include_invite_link", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :list], %{include_invite_link: true})
    ...
    test "include_account populates has_open_invitation based on first_login_at", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :list], %{include_account: true})
    ...
    test "include_company_access_levels", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :list], %{include_company_access_levels: true})
