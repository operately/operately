defmodule OperatelyWeb.Api.People.ListPossibleManagersTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:people, :list_possible_managers], user_id: "some-id")
    end
    ...
    test "returns people only from the company", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :list_possible_managers], user_id: Paths.person_id(ctx.creator))
    ...
    test "returns empty list for user_id from another company", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :list_possible_managers], user_id: Paths.person_id(other_user))
  ...
  describe "list_possible_managers functionality" do
    ...
    test "defaults to current user when user_id is not provided", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :list_possible_managers], [])
  ...
  describe "guest access control" do
    test "guest without view access returns empty list", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:people, :list_possible_managers], user_id: Paths.person_id(ctx.member1))
      assert res.people == []
