defmodule OperatelyWeb.Api.Spaces.DeleteTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:spaces, :delete], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company}, space=#{@test.space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:spaces, :delete], %{
  ...
  describe "functionality" do
    ...
    test "deletes space successfully", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:spaces, :delete], %{
        space_id: Paths.space_id(space),
      })
    ...
    test "returns an error when deleting the general space", ctx do
      ...
      assert {400, res} = mutation(ctx.conn, [:spaces, :delete], %{space_id: Paths.space_id(general_space)})
