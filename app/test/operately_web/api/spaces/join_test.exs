defmodule OperatelyWeb.Api.Spaces.JoinTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:spaces, :join], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "company members with #{@test.permissions} can #{if @test.expected == 200, do: "join", else: "not join"} space", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:spaces, :join], %{space_id: Paths.space_id(space)})
  ...
  describe "join_space functionality" do
    ...
    test "person joins space", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:spaces, :join], %{space_id: Paths.space_id(ctx.space)})
