defmodule OperatelyWeb.Api.Spaces.AddMembersTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:spaces, :add_members], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has company=#{@test.company} and space=#{@test.space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:spaces, :add_members], %{
  ...
  describe "add_space_members functionality" do
    ...
    test "adds one member", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:spaces, :add_members], %{
