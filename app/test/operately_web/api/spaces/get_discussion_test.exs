defmodule OperatelyWeb.Api.Spaces.GetDiscussionTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:spaces, :get_discussion], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    test "(company space) - company members have access", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:spaces, :get_discussion], %{id: Paths.message_id(message)})
      assert_message(res)
    end
    ...
    test "company members have no access", ctx do
      ...
      assert {404, %{message: msg} = _res} = query(ctx.conn, [:spaces, :get_discussion], %{id: Paths.message_id(message)})
  ...
  describe "space_discussions/get functionality" do
    ...
    test "include_unread_notifications", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:spaces, :get_discussion], %{
    ...
    test "include_space", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:spaces, :get_discussion], %{id: Paths.message_id(message), include_space: true})
    ...
    test "include_author", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:spaces, :get_discussion], %{id: Paths.message_id(message), include_author: true})
    ...
    test "include_reactions", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:spaces, :get_discussion], %{id: Paths.message_id(message), include_reactions: true})
    ...
    test "include_potential_subscribers", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:spaces, :get_discussion], %{id: Paths.message_id(ctx.message)})
