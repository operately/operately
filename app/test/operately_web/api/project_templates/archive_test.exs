defmodule OperatelyWeb.Api.ProjectTemplates.ArchiveTest do
  ...
  test "archives an active template", ctx do
    counts_before = runtime_side_effect_counts()
    assert {200, %{success: true}} = request(ctx)
    assert Repo.reload!(ctx.template).archived_at
  ...
  defp request(ctx), do: mutation(ctx.conn, [:project_templates, :archive], %{id: Paths.project_template_id(ctx.template)})
