defmodule OperatelyWeb.Api.ProjectTemplates.CreateDocumentTest do
  use OperatelyWeb.TurboCase

  alias Operately.ProjectTemplates.{ProjectTemplate, ResourceDocument, ResourceNode}
  ...
  test "creates a document in its parent folder", ctx do
    assert {200, %{document: document}} = request(ctx, %{parent_folder_id: Paths.project_template_resource_folder_id(ctx.folder), name: "Launch guide"})

    node = Repo.get_by!(ResourceNode, project_template_id: ctx.template.id, type: :document)
    assert node.parent_folder_id == ctx.folder.id
    assert Repo.get_by!(ResourceDocument, node_id: node.id).name == "Launch guide"
    assert document.name == "Launch guide"
  end
  ...
  tabletest @permissions_table do
    test "returns #{@test.expected} for #{@test.permissions}", ctx do
      ...
      assert {code, _} = request(ctx, %{name: "Requester document"})
  ...
  defp request(ctx, attrs \\ %{}) do
    mutation(
      ctx.conn,
      [:project_templates, :create_document],
      Map.merge(%{template_id: Paths.project_template_id(ctx.template), name: "Document", content: Jason.encode!(%{"type" => "doc", "content" => []})}, attrs)
    )
  end
end
