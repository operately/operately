defmodule OperatelyWeb.Api.Notifications.MarkManyAsReadTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:notifications, :mark_many_as_read], %{})
    end
    ...
    test "you can't mark someone else's notification as read", ctx do
      ...
      assert {404, res} = mutation(ctx.conn, [:notifications, :mark_many_as_read], %{ids: [n.id]})
  ...
  describe "mark_as_read functionality" do
    ...
    test "it marks notifications as read", ctx do
      ...
      assert {200, %{}} = mutation(ctx.conn, [:notifications, :mark_many_as_read], %{ids: [n1.id, n2.id]})
