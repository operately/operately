defmodule OperatelyWeb.Api.ResourceHubs.GetFolderTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:resource_hubs, :get_folder], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company} and space=#{@test.space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = query(ctx.conn, [:resource_hubs, :get_folder], %{
