defmodule OperatelyWeb.Api.ResourceHubs.ListNodesTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:resource_hubs, :list_nodes], %{})
    end
  end
  ...
  describe "goal-backed hubs" do
    ...
    test "fetches nodes for a goal-backed hub", ctx do
      assert {200, res} = query(ctx.conn, [:resource_hubs, :list_nodes], %{
        resource_hub_id: Paths.resource_hub_id(ctx.hub),
      })
