defmodule OperatelyWeb.Api.Queries.GlobalSearchTest do
  ...
  describe "global_search functionality" do
    ...
    test "returns empty results for queries less than 2 characters", ctx do
      ...
      assert res == %{projects: [], goals: [], tasks: [], people: [], work_map_link: nil}
    end
    ...
    test "searches tasks by name", ctx do
      ctx = 
        ...
        |> Factory.add_milestone(:launch, :website)
        |> Factory.add_task(:auth_task, :launch, name: "Implement authentication")
        |> Factory.add_task(:design_task, :launch, name: "Design homepage")
    ...
    test "returns work map link", ctx do
      ...
      assert res.work_map_link == "Company Work Map"
    end
