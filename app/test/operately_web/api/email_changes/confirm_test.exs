defmodule OperatelyWeb.Api.EmailChanges.ConfirmTest do
  ...
  test "requires authentication", ctx do
    assert {401, _} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: Ecto.UUID.generate(), code: "ABC123"})
    refute Map.has_key?(OperatelyWeb.Api.External.__mutations__(), "email_changes/confirm")
  end

  test "updates the current account only after verifying its code", ctx do
    ...
    assert {200, %{outcome: "invalid_code"}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: request.id, code: "wrong"})
    assert {200, %{outcome: "success", state: state}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: request.id, code: code})
    assert state.current_email == "new@example.com"
    assert state.pending == nil
  end

  test "a malformed request ID is a recoverable error", ctx do
    ctx = ctx |> Factory.setup() |> Factory.log_in_account(:account)
    assert {200, %{outcome: "request_invalid"}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: "invalid", code: "ABC123"})
  end
  ...
  test "rejects API tokens", ctx do
    ...
    assert {401, _} = mutation(conn, [:email_changes, :confirm], %{request_id: request.id, code: code})
  ...
  test "rejects missing, null and incorrectly typed inputs without consuming an attempt", ctx do
    ...
    for field <- [:request_id, :code], invalid <- [:missing, nil, 123, [], %{}] do
      ...
      assert {400, _} = mutation(ctx.conn, [:email_changes, :confirm], inputs)
    end
  ...
  test "rejects a supplied account identity", ctx do
    ...
    {:ok, request} = request_new_email(ctx.other, "other-new@example.com")
  ...
  test "cannot confirm another account's request even with the correct code", ctx do
    ctx = ctx |> Factory.setup() |> Factory.add_account(:other) |> Factory.log_in_account(:account)
    {:ok, other_request} = request_new_email(ctx.other, "other-new@example.com")
    other_code = delivered_code()
    {:ok, own_request} = request_new_email(ctx.account, "new@example.com")

    assert {200, %{outcome: "request_invalid", state: state}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: other_request.id, code: other_code})
  ...
  test "returns remaining attempts and leaves the email unchanged for an invalid code", ctx do
    ...
    assert {200, %{outcome: "invalid_code", state: state}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: request.id, code: "!!!!!!"})
    assert state.current_email == ctx.account.email
  ...
  test "returns code_expired with the pending state", ctx do
    ...
    assert {200, %{outcome: "code_expired", state: state}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: request.id, code: code})
    assert state.current_email == ctx.account.email
    assert state.pending.id == request.id
  ...
  test "returns too_many_attempts and refuses the correct code once exhausted", ctx do
    ...
    for code <- ["!!!!!!", code] do
      assert {200, %{outcome: "too_many_attempts", state: state}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: request.id, code: code})
      assert state.current_email == ctx.account.email
      assert state.pending.id == request.id
  ...
  test "returns email_taken when the destination is registered before confirmation", ctx do
    ...
    assert {200, %{outcome: "email_taken", state: state}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: request.id, code: code})
    assert state.current_email == ctx.account.email
    assert state.pending.id == request.id
    assert Repo.reload!(ctx.account).email == ctx.account.email
    assert Repo.reload!(request).invalidated_at == nil
  ...
  test "cannot confirm a cancelled request", ctx do
    ...
    assert {200, %{outcome: "request_invalid", state: state}} = mutation(ctx.conn, [:email_changes, :confirm], %{request_id: request.id, code: code})
    assert state.current_email == ctx.account.email
    assert state.pending == nil
    assert Repo.reload!(ctx.account).email == ctx.account.email
  end
  ...
  test "cannot confirm a replaced request and returns the replacement state", ctx do
    ...
    {:ok, replacement} = request_new_email(ctx.account, "different@example.com")
  ...
  test "cannot replay a consumed request and returns the updated account email", ctx do
    ctx = ctx |> Factory.setup() |> Factory.log_in_account(:account)
    {:ok, request} = request_new_email(ctx.account, "new@example.com")
    inputs = %{request_id: request.id, code: delivered_code()}
    assert {200, %{outcome: "success"}} = mutation(ctx.conn, [:email_changes, :confirm], inputs)

    assert {200, %{outcome: "request_invalid", state: state}} = mutation(ctx.conn, [:email_changes, :confirm], inputs)
    assert state.current_email == request.email
    assert state.pending == nil
    assert Repo.reload!(ctx.account).email == request.email
  end

  test "the current-inbox code cannot be submitted to final confirmation", ctx do
    ctx = ctx |> Factory.setup() |> Factory.log_in_account(:account)
