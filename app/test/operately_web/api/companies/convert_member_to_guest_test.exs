defmodule OperatelyWeb.Api.Companies.ConvertMemberToGuestTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:companies, :convert_member_to_guest], %{})
    end
  end
  ...
  defp request(ctx, person) do
    mutation(ctx.conn, [:companies, :convert_member_to_guest], %{person_id: Paths.person_id(person)})
  end
end
