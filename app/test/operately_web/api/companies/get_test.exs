defmodule OperatelyWeb.Api.Companies.GetTest do
  use OperatelyWeb.TurboCase

  test "invalid company id", ctx do
    assert {400, res} = query(ctx.conn, [:companies, :get], %{id: "invalid"})
  ...
  describe "security" do
    test "it requires authentication", ctx do
      ...
      assert {401, _} = query(ctx.conn, [:companies, :get], %{id: id})
    end
    ...
    test "suspended people don't have access", ctx do
      ...
      assert {200, res} = query(ctx.conn, [:companies, :get], %{id: Paths.company_id(ctx.company)})
      assert res.company == company

      {:ok, _} = Operately.People.update_person(ctx.person, %{suspended_at: DateTime.utc_now()})

      assert {404, res} = query(ctx.conn, [:companies, :get], %{id: Paths.company_id(ctx.company)})
  ...
  describe "get_company functionality" do
    ...
    test "include_people", ctx do
      assert {200, res} = query(ctx.conn, [:companies, :get], %{id: Paths.company_id(ctx.company), include_people: true})
    ...
    test "include_admins", ctx do
      assert {200, res} = query(ctx.conn, [:companies, :get], %{id: Paths.company_id(ctx.company), include_admins: true})
    ...
    test "include_owners", ctx do
      assert {200, res} = query(ctx.conn, [:companies, :get], %{id: Paths.company_id(ctx.company), include_owners: true})
