defmodule OperatelyWeb.Api.Companies.GetTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:companies, :get], %{})
    end
    ...
    test "suspended people don't have access", ctx do
      ...
      assert {404, res} = query(ctx.conn, [:companies, :get], %{})
  ...
  describe "get_company functionality" do
    ...
    test "include_people", ctx do
      assert {200, res} = query(ctx.conn, [:companies, :get], %{include_people: true})
    ...
    test "include_admins", ctx do
      assert {200, res} = query(ctx.conn, [:companies, :get], %{include_admins: true})
    ...
    test "include_owners", ctx do
      assert {200, res} = query(ctx.conn, [:companies, :get], %{include_owners: true})
