defmodule OperatelyWeb.Api.Companies.UpdateMembersPermissionsTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:companies, :update_members_permissions], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has access_level=#{@test.access_level}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:companies, :update_members_permissions], %{
          members: [%{
  ...
  describe "edit_company_members_permissions functionality" do
    ...
    test "edits one member's access level", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:companies, :update_members_permissions], %{
        members: [%{
          id: Paths.person_id(person),
          access_level: "edit_access",
        }],
    ...
    test "edits members to different access levels", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:companies, :update_members_permissions], %{
        members: [
  ...
  describe "access level validation" do
    ...
    test "admin cannot grant access higher than admin", ctx do
      assert {403, res} = mutation(ctx.conn, [:companies, :update_members_permissions], %{
        members: [%{
