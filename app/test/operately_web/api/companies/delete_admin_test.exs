defmodule OperatelyWeb.Api.Companies.DeleteAdminTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:companies, :delete_admin], %{})
    end
  end
  ...
  defp request(conn, person) do
    mutation(conn, [:companies, :delete_admin], %{person_id: Paths.person_id(person)})
  end
