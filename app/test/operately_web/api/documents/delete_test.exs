defmodule OperatelyWeb.Api.Documents.DeleteTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:documents, :delete], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company} and space=#{@test.space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:documents, :delete], %{
  ...
  describe "functionality" do
    ...
    test "removes document", ctx do
      assert {200, res} = mutation(ctx.conn, [:documents, :delete], %{
