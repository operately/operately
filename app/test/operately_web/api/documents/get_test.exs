defmodule OperatelyWeb.Api.Documents.GetTest do
  ...
  describe "get_resource_hub_document functionality" do
    ...
    test "include_versions_count", ctx do
      assert {200, res} = query(ctx.conn, [:documents, :get], %{id: Paths.document_id(ctx.doc)})
      assert is_nil(res.document.versions_count)

      assert {200, res} =
               query(ctx.conn, [:documents, :get], %{
                 id: Paths.document_id(ctx.doc),
                 include_versions_count: true
               })

      assert res.document.versions_count == 1
    end
    ...
    test "include_project returns the project-backed hub parent", ctx do
      ...
      refute res.document.resource_hub
      assert res.document.project.id == Paths.project_id(ctx.project)
      assert res.document.project.space.id == Paths.space_id(ctx.space)
    end
    ...
    test "include_goal returns the goal-backed hub data", ctx do
      ...
      refute res.document.resource_hub
      assert res.document.goal.id == Paths.goal_id(ctx.goal)
      assert res.document.goal.space.id == Paths.space_id(ctx.space)
    end
  end
