defmodule OperatelyWeb.Api.Files.UpdateTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:files, :update], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company} and space=#{@test.space}, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:files, :update], %{
  ...
  describe "functionality" do
    ...
    test "edits file", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:files, :update], %{
