defmodule OperatelyWeb.Api.GoalsTest do
  ...
  describe "update description" do
    ...
    test "it requires a goal_id", ctx do
      ...
      assert {422, res} = mutation(ctx.conn, [:goals, :update_description], %{description: "test"})
      assert res.errors == ["goal_id is required"]
    end

    test "it requires a description", ctx do
      ctx = Factory.log_in_person(ctx, :creator)

      assert {422, res} = mutation(ctx.conn, [:goals, :update_description], %{goal_id: "test"})
      assert res.errors == ["description is required"]
    end
  end
