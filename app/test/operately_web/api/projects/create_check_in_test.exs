defmodule OperatelyWeb.Api.Projects.CreateCheckInTest do
  ...
  describe "post_project_check_in functionality" do
    ...
    test "saves project check-in as a draft without publishing side effects", ctx do
      project = project_fixture(%{company_id: ctx.company.id, creator_id: ctx.person.id, group_id: ctx.company.company_space_id})
      ...
      assert {200, res} =
               mutation(ctx.conn, [:projects, :create_check_in], %{
                 project_id: Paths.project_id(project),
                 status: "caution",
                 description: RichText.rich_text("Draft description", :as_string),
                 post_as_draft: true
               })
      ...
      assert res.check_in.state == "draft"
