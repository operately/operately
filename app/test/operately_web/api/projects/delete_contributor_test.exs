defmodule OperatelyWeb.Api.Projects.DeleteContributorTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:projects, :delete_contributor], %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company}, space=#{@test.space}, project=#{@test.project} on the project, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, [:projects, :delete_contributor], %{contrib_id: contributor.id})
  ...
  describe "remove_project_contributor functionality" do
    ...
    test "removes project contributor", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:projects, :delete_contributor], %{contrib_id: contributor.id})

      assert res.contributor == Serializer.serialize(Repo.preload(contributor, :person), level: :essential)
      refute from(c in Projects.Contributor, where: c.id == ^contributor.id) |> Repo.one()
    end
  end

  describe "can remove contributors validation" do
    @can_remove_table [
    ...
    tabletest @can_remove_table do
      test "user with #{@test.caller_access} can remove contributor with #{@test.contributor_access}, expect code=#{@test.expected}", ctx do
        space = create_space(ctx)
        ...
        assert {code, res} = mutation(conn, [:projects, :delete_contributor], %{contrib_id: Paths.project_contributor_id(target)})

        assert code == @test.expected

        case @test.expected do
          200 -> refute from(c in Projects.Contributor, where: c.id == ^target.id) |> Repo.one()
          403 -> assert res.message == "You don't have permission to perform this action"
        end
      end
    end
