defmodule OperatelyWeb.Api.Projects.MilestonesTest do
  ...
  describe "list tasks" do
    ...
    test "it requires authentication", ctx do
      assert {401, _} = query(ctx.conn, [:projects, :list_milestone_tasks], %{})
    end

    test "it requires a milestone_id", ctx do
      ctx = Factory.log_in_person(ctx, :creator)

      assert {400, res} = query(ctx.conn, [:projects, :list_milestone_tasks], %{})
    ...
    test "it returns not found for non-existent milestone", ctx do
      ...
      assert {404, _} = query(ctx.conn, [:projects, :list_milestone_tasks], %{
    ...
    test "it works with milestones from different projects", ctx do
      ...
      assert {200, res1} = query(ctx.conn, [:projects, :list_milestone_tasks], %{
        milestone_id: Paths.milestone_id(ctx.milestone)
      })

      # Query tasks from second milestone
      assert {200, res2} = query(ctx.conn, [:projects, :list_milestone_tasks], %{
  ...
  describe "update kanban" do
    ...
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:projects, :update_milestone_kanban], %{})
    end
    ...
    test "it updates status and kanban state for a milestone", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:projects, :update_milestone_kanban], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
    ...
    test "it rejects invalid statuses in kanban state", ctx do
      ...
      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_kanban], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        task_id: Paths.task_id(ctx.task),
        status: status_input,
    ...
    test "it updates milestone tasks_kanban_state and only creates activity when task status changes", ctx do
      ...
      assert {200, res1} = mutation(ctx.conn, [:projects, :update_milestone_kanban], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        task_id: Paths.task_id(ctx.task),
        status: status_input,
        kanban_state: Jason.encode!(kanban_state)
      ...
      assert {200, _res2} = mutation(ctx.conn, [:projects, :update_milestone_kanban], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        task_id: Paths.task_id(ctx.task),
        status: status_input,
        kanban_state: Jason.encode!(kanban_state)
    ...
    test "it doesn't create an activity when only the order changes", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:projects, :update_milestone_kanban], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
    ...
    test "it doesn't work when the task doesn't belong to the milestone project", ctx do
      ...
      assert {404, _} = mutation(ctx.conn, [:projects, :update_milestone_kanban], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
  ...
  describe "update title" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:projects, :update_milestone_title], %{})
    end

    test "it requires a milestone_id", ctx do
      ctx = Factory.log_in_person(ctx, :creator)

      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_title], %{title: "New Title"})
      assert res.message == "Missing required fields: milestone_id"
    end
    ...
    test "it requires a title", ctx do
      ...
      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_title], %{milestone_id: Paths.milestone_id(ctx.milestone)})
    ...
    test "it returns not found for non-existent milestone", ctx do
      ...
      assert {404, _} = mutation(ctx.conn, [:projects, :update_milestone_title], %{
        milestone_id: Ecto.UUID.generate(),
        title: "New Title"
      })
    end
    ...
    test "it returns forbidden for non-space-members", ctx do
      ...
      assert {403, _} = mutation(ctx.conn, [:projects, :update_milestone_title], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        title: "New Title"
      })
    end
    ...
    test "it updates the milestone title for project creator", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:projects, :update_milestone_title], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
    ...
    test "it preserves other milestone fields when updating title", ctx do
      ...
      assert {200, _res} = mutation(ctx.conn, [:projects, :update_milestone_title], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
    ...
    test "it validates title is not empty", ctx do
      ...
      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_title], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        title: ""
      })
    ...
    test "it creates an activity when title is updated", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:projects, :update_milestone_title], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
    ...
    test "it works with milestones from different projects", ctx do
      ...
      assert {200, res1} = mutation(ctx.conn, [:projects, :update_milestone_title], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        title: "Project 1 Milestone"
      })

      # Update milestone from second project
      assert {200, res2} = mutation(ctx.conn, [:projects, :update_milestone_title], %{
        milestone_id: Paths.milestone_id(ctx.other_milestone),
  ...
  describe "update due date" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:projects, :update_milestone_due_date], %{})
    end

    test "it requires a milestone_id", ctx do
      ctx = Factory.log_in_person(ctx, :creator)

      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_due_date], %{due_date: %{date: "2026-01-01", date_type: "day"}})
      assert res.message == "Missing required fields: milestone_id"
    end
    ...
    test "it returns not found for non-existent milestone", ctx do
      ...
      assert {404, _} = mutation(ctx.conn, [:projects, :update_milestone_due_date], %{
        milestone_id: Ecto.UUID.generate(),
    ...
    test "it returns forbidden for non-space-members", ctx do
      ...
      assert {403, _} = mutation(ctx.conn, [:projects, :update_milestone_due_date], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        due_date: %{date: "2026-01-01", date_type: "day"}
      })
    end
    ...
    test "it updates the milestone due date for project creator", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:projects, :update_milestone_due_date], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
    ...
    test "it preserves start date when updating due date", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:projects, :update_milestone_due_date], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
    ...
    test "it sends notifications to subscribed people", ctx do
      ...
      {200, _} = Oban.Testing.with_testing_mode(:manual, fn ->
        mutation(ctx.conn, [:projects, :update_milestone_due_date], %{
          milestone_id: Paths.milestone_id(ctx.milestone),
          due_date: due_date
        })
      end)
  ...
  describe "update description" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:projects, :update_milestone_description], %{})
    end

    test "it requires a milestone_id", ctx do
      ctx = Factory.log_in_person(ctx, :creator)

      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_description], %{description: RichText.rich_text("Test description", :as_string)})
      assert res.message == "Missing required fields: milestone_id"
    end
    ...
    test "it requires a description", ctx do
      ...
      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_description], %{milestone_id: Paths.milestone_id(ctx.milestone)})
    ...
    test "it returns not found for non-existent milestone", ctx do
      ...
      assert {404, _} = mutation(ctx.conn, [:projects, :update_milestone_description], %{
        milestone_id: Ecto.UUID.generate(),
    ...
    test "it returns forbidden for non-space-members", ctx do
      ...
      assert {403, _} = mutation(ctx.conn, [:projects, :update_milestone_description], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        description: RichText.rich_text("Test description", :as_string)
      })
    end
    ...
    test "Project creator can update milestone description", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:projects, :update_milestone_description], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
    ...
    test "it preserves other milestone fields when updating description", ctx do
      ...
      assert {200, _res} = mutation(ctx.conn, [:projects, :update_milestone_description], %{
        milestone_id: Paths.milestone_id(ctx.milestone),
        description: Jason.encode!(description)
      })
    ...
    test "it sends notifications to mentioned people", ctx do
      ...
      {200, _} = Oban.Testing.with_testing_mode(:manual, fn ->
        mutation(ctx.conn, [:projects, :update_milestone_description], %{
          milestone_id: Paths.milestone_id(ctx.milestone),
          description: description
        })
      end)
  ...
  describe "update ordering" do
    ...
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:projects, :update_milestone_ordering], %{})
    end

    test "it requires a project_id", ctx do
      ctx = Factory.log_in_person(ctx, :creator)

      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_ordering], %{ordering_state: []})
    ...
    test "it returns not found for non-existent project", ctx do
      ...
      assert {404, _} = mutation(ctx.conn, [:projects, :update_milestone_ordering], %{
    ...
    test "it returns forbidden for members without edit access", ctx do
      ...
      assert {403, _} = mutation(ctx.conn, [:projects, :update_milestone_ordering], %{
    ...
    test "it rejects milestones from other projects", ctx do
      ...
      assert {400, res} = mutation(ctx.conn, [:projects, :update_milestone_ordering], %{
        project_id: Paths.project_id(ctx.project),
        ordering_state: [
    ...
    test "it appends missing milestone ids when saving", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:projects, :update_milestone_ordering], %{
        project_id: Paths.project_id(ctx.project),
  ...
  describe "delete" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, [:projects, :delete_milestone], %{})
    end

    test "it requires a milestone_id", ctx do
      ctx = Factory.log_in_person(ctx, :creator)

      assert {400, res} = mutation(ctx.conn, [:projects, :delete_milestone], %{})
      assert res.message == "Missing required fields: milestone_id"
    end
    ...
    test "it returns not found for non-existent milestone", ctx do
      ...
      assert {404, _} = mutation(ctx.conn, [:projects, :delete_milestone], %{
        milestone_id: Ecto.UUID.generate()
      })
    end
    ...
    test "it returns forbidden for non-space-members", ctx do
      ...
      assert {403, _} = mutation(ctx.conn, [:projects, :delete_milestone], %{
        milestone_id: Paths.milestone_id(ctx.milestone)
      })
    end
    ...
    test "it deletes the milestone for project creator", ctx do
      ...
      assert {200, res} = mutation(ctx.conn, [:projects, :delete_milestone], %{
        milestone_id: Paths.milestone_id(ctx.milestone)
      })
    ...
    test "it creates an activity when milestone is deleted", ctx do
      ...
      assert {200, _} = mutation(ctx.conn, [:projects, :delete_milestone], %{
        milestone_id: Paths.milestone_id(ctx.milestone)
      })
    ...
    test "it works with milestones from different projects", ctx do
      ...
      assert {200, res1} = mutation(ctx.conn, [:projects, :delete_milestone], %{
        milestone_id: Paths.milestone_id(ctx.milestone)
      })

      # Delete milestone from second project
      assert {200, res2} = mutation(ctx.conn, [:projects, :delete_milestone], %{
        milestone_id: Paths.milestone_id(ctx.other_milestone)
      })
