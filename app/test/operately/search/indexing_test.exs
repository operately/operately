defmodule Operately.Search.IndexingTest do
  ...
  test "upserts normalized searchable entries and preserves their identity", ctx do
    ...
    assert {:ok, %{inserted: 1, updated: 0, unchanged: 0, invalid: 0, invalid_entries: []}} = Indexing.upsert(attrs)
    ...
    assert {:ok, %{inserted: 0, updated: 1, unchanged: 0, invalid: 0, invalid_entries: []}} =
             Indexing.upsert(%{attrs | title: "Launch plan", body: "Updated details"})
  ...
  test "reports unchanged and invalid entries without discarding valid entries", ctx do
    ...
    assert {:ok, _} = Indexing.upsert(attrs)
    ...
    assert {:ok, %{inserted: 0, updated: 0, unchanged: 1, invalid: 1, invalid_entries: [error]}} =
             Indexing.upsert_all([attrs, invalid])
  ...
  test "guarded upserts do not replace newer entries with late maintenance snapshots", ctx do
    ...
    assert {:ok, %{inserted: 1}} = Indexing.upsert(newer_attrs)
    assert {:ok, %{updated: 0, superseded: 1}} = Indexing.upsert_guarded(old_attrs)
  ...
  test "guarded upserts keep an equal-version entry when indexed data differs", ctx do
    ...
    assert {:ok, %{inserted: 1}} = Indexing.upsert(attrs)

    assert {:ok, %{updated: 0, superseded: 1}} =
             Indexing.upsert_guarded(%{attrs | title: "Ambiguous maintenance value"})
  ...
  test "guarded upserts accept newer source versions", ctx do
    ...
    assert {:ok, %{updated: 1, superseded: 0}} = Indexing.upsert_guarded(newer_attrs)
  ...
  test "locked repairs fix projection corruption with an equal source version", ctx do
    attrs = entry_attrs(ctx, title: "Canonical value")
    assert {:ok, %{inserted: 1}} = Indexing.upsert(%{attrs | title: "Stale projection"})

    assert {:ok, %{updated: 1, superseded: 0}} = Indexing.repair_locked(attrs)
    assert Repo.get_by!(Entry, source_id: ctx.project.id).title == "Canonical value"
  end
  ...
  test "requires a source update timestamp", ctx do
    ...
    assert {:ok, %{invalid: 1, invalid_entries: [error]}} = Indexing.upsert(attrs)
  ...
  test "deletes entries directly and through an Ecto.Multi", ctx do
    assert {:ok, _} = Indexing.upsert(entry_attrs(ctx))
    assert {:ok, 1} = Indexing.delete("project", ctx.project.id)

    assert {:ok, _} = Indexing.upsert(entry_attrs(ctx))

    assert {:ok, %{search_entry: 1}} =
             Multi.new()
             |> Indexing.delete(:search_entry, "project", ctx.project.id)
             |> Repo.transaction()
  ...
  test "bulk operations participate in an Ecto.Multi", ctx do
    ...
    assert {:ok, %{upsert: %{inserted: 1}, delete: 1}} =
             Multi.new()
             |> Indexing.upsert_all(:upsert, [attrs])
             |> Indexing.delete_many(:delete, [{attrs.source_type, attrs.source_id}])
             |> Repo.transaction()
  ...
  test "uses an empty body when an adapter omits or clears it", ctx do
    ...
    assert {:ok, %{updated: 0, unchanged: 1}} = Indexing.upsert(Map.put(attrs, :body, nil))
  end
  ...
  test "casts persisted display states to atoms", ctx do
    ...
    assert {:ok, %{invalid: 1, invalid_entries: [error]}} = Indexing.upsert(invalid_attrs)
  ...
  test "Ecto.Multi upserts participate in the surrounding transaction", ctx do
    result =
      Multi.new()
      |> Indexing.upsert(:search_entry, entry_attrs(ctx))
  ...
  test "Ecto.Multi upserts can build entries from earlier results", ctx do
    ...
    assert {:ok, %{search_entry: %{inserted: 1}}} =
             ...
             |> Indexing.upsert(:search_entry, fn %{source_attrs: source_attrs} -> source_attrs end)
             |> Repo.transaction()
  ...
  test "guarded Ecto.Multi upserts roll back with their transaction", ctx do
    result =
      Multi.new()
      |> Indexing.upsert_guarded(:search_entry, entry_attrs(ctx))
      |> Multi.run(:forced_failure, fn _, _ -> {:error, :rollback} end)
      |> Repo.transaction()
