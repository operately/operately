defmodule Operately.Search.Indexing.Adapters.ResourceHub.AdapterContractTest do
  ...
  alias Operately.Search.Indexing.Adapters.ResourceHub.{Document, File, Folder, Link}
  ...
  test "registers every resource hub source adapter" do
    assert {:ok, Folder} = AdapterRegistry.fetch("resource_hub_folder")
    assert {:ok, Document} = AdapterRegistry.fetch("resource_hub_document")
    assert {:ok, File} = AdapterRegistry.fetch("resource_hub_file")
    assert {:ok, Link} = AdapterRegistry.fetch("resource_hub_link")
  end
