defmodule Operately.Search.Indexing.ResourceHubTest do
  ...
  test "continues jobs queued with the previous worker module name", ctx do
    job =
      %Oban.Job{
        args: %{
          "source_type" => "resource_hub_folder",
          "source_ids" => [ctx.first_folder.id]
        }
      }

    assert {:ok, Operately.Search.ResourceHubIndex.Worker} = Oban.Worker.from_string("Operately.Search.ResourceHubIndex.Worker")
    assert :ok = Operately.Search.ResourceHubIndex.Worker.perform(job)
    assert_entry(:resource_hub_folder, ctx.first_folder.id)
  end
