defmodule Operately.Search.Indexing.SynchronizerTest do
  ...
  alias Operately.Search.Indexing.{AdapterRegistry, ResourceHub, Synchronizer}
  alias Operately.Search.Indexing.ResourceHub.Worker
  ...
  test "reloads the source and authoritatively synchronizes its entry", ctx do
    assert {:ok, %{inserted: 1, deleted: 0}} = Synchronizer.sync("resource_hub_folder", ctx.folder.id)
    ...
    assert {:ok, %{updated: 1, deleted: 0}} = Synchronizer.sync("resource_hub_folder", ctx.folder.id)
  ...
  test "deletes entries for missing or excluded sources", ctx do
    assert {:ok, _summary} = Synchronizer.sync("resource_hub_folder", ctx.folder.id)
    Repo.soft_delete!(ctx.folder)

    assert {:ok, %{deleted: 1}} = Synchronizer.sync("resource_hub_folder", ctx.folder.id)
    refute Repo.get_by(Entry, source_id: ctx.folder.id)

    missing_id = Ecto.UUID.generate()
    assert {:ok, %{deleted: 0}} = Synchronizer.sync("resource_hub_folder", missing_id)
  end
  ...
  test "canonical writes commit before a queued indexing failure is retried", ctx do
    ...
    Application.put_env(:operately, AdapterRegistry, [InvalidFolderSource])
    ...
    Oban.Testing.with_testing_mode(:manual, fn ->
      assert {:ok, _changes} =
               ...
               |> ResourceHub.enqueue_resource(:search_folder, :folder, fn changes -> changes.folder.id end)
               |> Repo.transaction()
  ...
  test "validates the complete batch before writing any entry", ctx do
    previous_sources = Application.fetch_env(:operately, AdapterRegistry)
    Application.put_env(:operately, AdapterRegistry, [PreparedFolderSource])
    on_exit(fn -> restore_source_registry(previous_sources) end)
    ...
    assert {:error, :invalid_search_entry} =
             Synchronizer.sync_all("resource_hub_folder", [valid_id, invalid_id])
