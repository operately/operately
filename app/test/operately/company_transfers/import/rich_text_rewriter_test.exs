defmodule Operately.CompanyTransfers.Import.RichTextRewriterTest do
  ...
  test "rewrites mentions and blobs in project template documents" do
    source_person = person("Template Mention")
    destination_person_id = Ecto.UUID.generate()
    source_blob_id = Ecto.UUID.generate()
    destination_blob_id = Ecto.UUID.generate()
    ...
    assert map_fields_for("project_template_resource_documents") == ["content"]

    assert {:ok, rewritten} =
             RichTextRewriter.rewrite_row_mentions(
               row,
               "project_template_resource_documents",
               plan,
               map_fields_for("project_template_resource_documents")
             )

    assert RichContent.find_mentioned_ids(rewritten["content"]) == [
             encoded_person_id("Template Mention", destination_person_id)
           ]
