defmodule Operately.CompanyTransfers.Export.FileArchiveTest do
  ...
  test "create!/3 skips blobs whose storage file is missing", ctx do
    ...
    File.write!(json_path, ~s({"tables":[]}))

    on_exit(fn ->
      cleanup_paths([source_path, json_path, storage_path(present_blob)])
    end)
    ...
    assert ^zip_path = FileArchive.create!(zip_path, json_path, files)

    extracted = Archive.extract!(zip_path, extract_dir)

    assert Path.join(extract_dir, "files/blobs/#{present_blob.id}/#{present_blob.filename}") in extracted
    refute Path.join(extract_dir, "files/blobs/#{missing_blob.id}/#{missing_blob.filename}") in extracted
  end

  test "create!/3 succeeds when all blob storage files are missing", ctx do
    ...
    on_exit(fn -> cleanup_paths([json_path, zip_path]) end)
