defmodule Operately.CompanyTransfers.Export.RelationalCollectorTest do
  ...
  test "collects a company's project template graph without leaking another company", ctx do
    ctx =
      ...
      |> Factory.add_project_template_comment(:template_comment, :template, :template_discussion)
    ...
    assert ctx.template_comment.id in row_ids(tables, "project_template_comments")
    ...
    refute other_ctx.template_comment.id in row_ids(tables, "project_template_comments")
