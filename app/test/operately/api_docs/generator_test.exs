defmodule Operately.ApiDocs.GeneratorTest do
  ...
  test "generates one page per external endpoint plus namespace indexes", %{out_dir: out_dir} do
    ...
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/get_document.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/get_file.mdx"))
  end
  ...
  test "hidden external endpoints are routable but excluded from catalog and docs", %{out_dir: out_dir} do
    ...
    assert Map.has_key?(OperatelyWeb.Api.External.__mutations__(), "docs_and_files/update_file")
    assert Map.has_key?(OperatelyWeb.Api.External.__queries__(), "docs_and_files/get_file")
    ...
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "docs_and_files/get_file"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "docs_and_files/update_file"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "docs_and_files/delete_file"))
    ...
    refute File.exists?(Path.join(out_dir, "help/api/files/delete.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/get_file.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/update_file.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/delete_file.mdx"))
    ...
    assert Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "docs_and_files/get_file"))
    assert Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "docs_and_files/update_file"))
    assert Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "docs_and_files/delete_file"))
  end
  ...
  test "legacy endpoints with docs_and_files wrappers are routable but excluded from catalog and docs", %{out_dir: out_dir} do
    ...
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "docs_and_files/get_document"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "docs_and_files/get_link"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "docs_and_files/get_file"))
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "docs_and_files/create_file"))
    ...
    refute File.exists?(Path.join(out_dir, "help/api/documents/index.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/documents/create.mdx"))
    ...
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/create_link.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/get_document.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/get_link.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/docs_and_files/get_file.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/docs_and_files/create_file.mdx"))
  end
