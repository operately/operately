defmodule Operately.ApiDocs.GeneratorTest do
  ...
  test "generates one page per external endpoint plus namespace indexes", %{out_dir: out_dir} do
    ...
    refute File.exists?(Path.join(out_dir, "help/api/files/get.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/files/update.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/files/delete.mdx"))
    ...
    assert File.exists?(Path.join(out_dir, "help/api/documents/create_document.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/create_link.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/get_document.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/get_file.mdx"))
  end
  ...
  test "hidden external endpoints are routable but excluded from catalog and docs", %{out_dir: out_dir} do
    ...
    assert Map.has_key?(OperatelyWeb.Api.External.__mutations__(), "documents/update_file")
    assert Map.has_key?(OperatelyWeb.Api.External.__queries__(), "documents/get_file")
    ...
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "files/get"))
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "files/update"))
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "files/delete"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/get_file"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/update_file"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/delete_file"))
    ...
    refute File.exists?(Path.join(out_dir, "help/api/files/delete.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/get_file.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/update_file.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/delete_file.mdx"))
    ...
    refute Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "files/get"))
    refute Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "files/update"))
    refute Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "files/delete"))
    assert Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "documents/get_file"))
    assert Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "documents/update_file"))
    assert Enum.any?(Catalog.payload(catalog, "/api/external/v1").endpoints, &(&1.full_name == "documents/delete_file"))
  end

  test "legacy endpoints with documents wrappers are routable but excluded from catalog and docs", %{out_dir: out_dir} do
    ...
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "files/create"))
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "documents/get"))
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "links/get"))
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "files/get"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/list_contents"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/create_folder"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/get_folder"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/copy_folder"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/create_document"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/create_link"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/get_document"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/get_link"))
    assert Enum.any?(catalog.endpoints, &(&1.full_name == "documents/get_file"))
    refute Enum.any?(catalog.endpoints, &(&1.full_name == "documents/create_file"))
    ...
    refute File.exists?(Path.join(out_dir, "help/api/documents/create.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/documents/get.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/links/index.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/links/create.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/files/index.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/files/create.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/index.mdx"))
    refute File.exists?(Path.join(out_dir, "help/api/documents/get.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/create_folder.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/get_folder.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/copy_folder.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/create_document.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/create_link.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/get_document.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/get_link.mdx"))
    assert File.exists?(Path.join(out_dir, "help/api/documents/get_file.mdx"))
