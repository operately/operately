defmodule Operately.MD.SpaceTest do
  ...
  test "it renders loaded company and members without tasks" do
    ...
    ai = person_fixture(%{company_id: company.id, full_name: "Assistant AI", type: :ai})
    space = group_fixture(creator, %{company_id: company.id, name: "Roadmap Space", mission: "Coordinate roadmap work"})
    Operately.Groups.add_members(creator, space.id, [%{id: guest.id, access_level: Operately.Access.Binding.comment_access()}, %{id: ai.id, access_level: Operately.Access.Binding.comment_access()}])
