defmodule Operately.Data.Change098SetPersonNotificationDefaultsTest do
  use Operately.DataCase
  import Ecto.Query

  alias Operately.Repo
  alias Operately.Data.Change098SetPersonNotificationDefaults, as: Change
  alias Operately.Data.Change098SetPersonNotificationDefaults.Person
  ...
  test "run/0 sets notify_on_mention true without changing send_daily_summary", ctx do
    ...
    assert map_get(creator_notifications, "send_daily_summary") == true
    assert map_get(creator_notifications, "notify_about_assignments") == false
    ...
    assert map_get(member_one_notifications, "send_daily_summary") == true

    member_two_notifications = fetch_notifications(ctx.member_two.id)
    assert map_get(member_two_notifications, "notify_on_mention") == true
    assert map_get(member_two_notifications, "send_daily_summary") == nil
    assert map_get(fetch_preferences(ctx.member_two.id), "appearance") == %{"theme" => "light"}
  end

  test "set_notification_defaults/1 forces notify_on_mention and preserves other notification fields" do
    preferences =
      Change.set_notification_defaults(%{
        notifications: %{
          notify_on_mention: false,
          send_daily_summary: true,
          notify_about_assignments: false
    ...
    assert map_get(notifications, "send_daily_summary") == true
    assert map_get(notifications, "notify_about_assignments") == false
  end
