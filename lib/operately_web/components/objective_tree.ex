defmodule OperatelyWeb.ObjectiveTree do
  ...
  def objective_tree(assigns) do
    ...
    objectives = Tree.to_list_preorder(tree)

    assigns = %{objectives: objectives}

    ~H"""
      <%= for {objective, depth} <- @objectives do %>
        <%= objective_tree_node(%{objective: objective, depth: depth}) %>
      <% end %>
    """
  end

  defp objective_tree_node(%{objective: objective, depth: depth} = assigns) do
