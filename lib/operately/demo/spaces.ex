defmodule Operately.Demo.Spaces do
  ...
  def create_spaces(context) do
    ...
    {:ok, marketing_space} = create_space(%{
      creator: context.owner,
      name: "Marketing",
    ...
    {:ok, company_space} = find_company_space(context.company)

    context
    |> Map.put(:company_space, company_space)
  ...
  defp create_space(%{creator: creator, name: name, description: mission, icon: icon, color: color}) do
    Operately.Groups.create_group(creator, %{
      name: name,
      mission: mission,
      icon: icon,
      color: color,
