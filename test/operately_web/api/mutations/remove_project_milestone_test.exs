defmodule OperatelyWeb.Api.Mutations.RemoveProjectMilestoneTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, :remove_project_milestone, %{})
    end
  end
  ...
  describe "permissions" do
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company}, space=#{@test.space}, project=#{@test.project} on the project, then expect code=#{@test.expected}", ctx do
        ...
        assert {code, res} = mutation(ctx.conn, :remove_project_milestone, %{milestone_id: Paths.milestone_id(milestone)})

        assert code == @test.expected

        case @test.expected do
          200 -> assert %{milestone: _} = res
