defmodule OperatelyWeb.Api.Mutations.UpdateMilestoneDescriptionTest do
  ...
  describe "security" do
    test "it requires authentication", ctx do
      assert {401, _} = mutation(ctx.conn, :update_milestone_description, %{})
    end
  end
    ...
    tabletest @table do
      test "if caller has levels company=#{@test.company}, space=#{@test.space}, project=#{@test.project} on the project, then expect code=#{@test.expected}", ctx do
        ...
        case @test.expected do
          200 -> assert res == %{milestone: Serializer.serialize(milestone, level: :essential)}
