...
export function DropdownMenu({ open, setOpen, trigger, options }: DropdownMenuProps) {
...
export function DropdownMenuLinkItem({ to, title }: { to: string; title: string }) {
  return (
    <DivLink to={to} className="hover:bg-accent-1 hover:text-white-1 rounded-md px-1.5 py-0.5 text-sm cursor-pointer">
      {title}
    </DivLink>
  );
}
