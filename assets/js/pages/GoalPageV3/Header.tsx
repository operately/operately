  ...
  if (goal) {
    content = (
      <div className="flex items-center gap-1">
        <Icons.IconTarget size={14} className="text-content-dimmed" />
...
function Timeframe({ goal }: { goal: Goals.Goal }) {
  ...
  return (
    <div className="font-medium text-sm text-content-dimmed">
...
function TimeframeState({ goal }) {
  ...
  if (Timeframes.isOverdue(timeframe)) {
    ...
    return (
      <span>
        <span className="ml-2 text-accent-1">{plurarize(Timeframes.remainingDays(timeframe), "day", "days")} left</span>
      </span>
    );
  }
}
