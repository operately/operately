...
export function Status({ node, children }: { node: Node; children: ReactNode }) {
  ...
  if (node.type === "project" && !node.isClosed) {
...
function useClickHandler(node: Node) {
  ...
  return () => {
    if (node.type === "goal") {
      if (node.isClosed) {
        navigate(Paths.goalPath(node.id!));
      } else {
        navigate(Paths.goalProgressUpdatePath(node.asGoalNode()!.lastCheckIn!.id!));
      }
    }

    if (node.type === "project") {
      if (node.isClosed) {
        navigate(Paths.projectRetrospectivePath(node.id!));
      } else {
        navigate(Paths.projectCheckInPath(node.asProjectNode()!.lastCheckIn!.id!));
      }
    }
...
function NodeStatusIndicator({ node }: { node: Node }) {
  ...
  if (node.type === "goal") {
    if (node.isClosed) {
      ...
      const status = node.asGoalNode().lastCheckIn!.status!;
      return <SmallStatusIndicator status={status} size="sm" textClassName="text-content-dimmed" />;
    }
  }
